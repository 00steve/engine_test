/*
the fpse camera is a camera with unique controls suitable
for editing in fpse. The right mouse button is held when
the user wants to rotate the camera, otherwise the mouse
is moved around the environment.

*/
#ifndef FPSE_CAMERA_H
#define FPSE_CAMERA_H

#include "../core/camera.h"

class fpse_camera : public camera{
    protected:
        quaternion rotation;
        double3 offset;
        float fov;
        ...
        void internal_update(){
            ...
            mov = mov.normalize()*.15f;
            offset = offset + double3(mov.y*sin(angle.y)*cos(angle.x)+mov.x*cos(angle.y)*cos(angle.x), mov.y*sin(angle.x), mov.y*cos(angle.y)*cos(angle.x)-mov.x*sin(angle.y)*cos(angle.x) );
        }
    ...
    public:
		...
		fpse_camera(varMap settings) :
			camera(settings.isSet("id") ? new string(settings.getString("id","")) : NULL),
			offset(settings.getDouble3("position",double3(0,0,0))),
